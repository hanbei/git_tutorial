...
\begin{columns}[T]
  \begin{column}{.45\textwidth}
    \begin{block}{}
      \includegraphics<1>[scale=0.4]{images/branches/master-start.pdf}
      \includegraphics<2>[scale=0.4]{images/branches/first-branch.pdf}
	  \includegraphics<3>[scale=0.4]{images/branches/back-to-master.pdf}
	  \includegraphics<4>[scale=0.4]{images/branches/switch-branch.pdf}
	  \includegraphics<5>[scale=0.4]{images/branches/work-branch.pdf}
	  \includegraphics<6>[scale=0.4]{images/branches/work-master.pdf}
	  \includegraphics<7>[scale=0.4]{images/branches/branch-merge.pdf}
    \end{block}
  \end{column}
  \begin{column}{.55\textwidth}
    ...
    \begin{tiny}
    	\begin{itemize}
    	  \item \texttt{@master}
    	  \pause \item \texttt{git checkout -b iss91}
    	  \item \texttt{git commit}
      	  \pause \item \texttt{git checkout master}
      	  \pause \item \texttt{git commit}
      	  \item \texttt{git checkout iss91}
      	  \pause \item \texttt{git commit}
		  \pause \item \texttt{git checkout master}
		  \pause \item \texttt{git commit}
		  \item \texttt{git merge iss91}
		\end{itemize}
	  \end{tiny}
    \end{block}
  \end{column}
...
\begin{columns}[T]
  \begin{column}{.45\textwidth}
    \begin{block}{}
      \includegraphics[scale=0.4]{images/branches/feature-branches1.pdf}
    \end{block}
  \end{column}
...
\begin{frame}
  \begin{itemize}
    \item Git Workflow http://nvie.com/posts/a-successful-git-branching-model/%20gitflow%20branching%20model
  \end{itemize}
\end{frame}
